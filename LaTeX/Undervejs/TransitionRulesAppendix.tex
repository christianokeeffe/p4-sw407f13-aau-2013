...
~ & \indent\indent and $env_V \; [r[v_1] = l_1$ \\
...
\dfrac{env_V'[\vec{z} \mapsto \vec{l}][\text{next} \mapsto l'], env_P' \vdash \langle S, sto \rangle \rightarrow sto'}{env_V, env_P \vdash \langle \text{call} \; p(\vec{x}), sto \rangle \rightarrow sto'}$\\
~ & ~ \\
~ & \indent\indent where $env_V \; \vec{x} = \vec{l}$ \\
~ & \indent\indent and $env_P \; p = (S, \vec{z}, env_V', env_P')$ \\
~ & \indent\indent and $|\vec{x}| = |\vec{z}|$ \\
~ & \indent\indent and $env_V, next = l'$ \\
~ & ~ \\
...
[ARRAY-DEC] & $\dfrac{\langle D_A, env_V[r \mapsto l, \text{next} \mapsto l + v + 1],  sto[l \mapsto r] \rangle \rightarrow_{DA} (env_V', sto')}{\langle ??? r[a_1], D_A, env_V, sto \rangle \rightarrow_{DA} (env_V', sto')}$ \\
~ & ~ \\
