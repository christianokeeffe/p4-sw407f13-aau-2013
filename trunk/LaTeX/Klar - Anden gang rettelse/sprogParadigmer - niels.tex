...
The code \ref{lst:FunctionalEKS} will always be true in functional programming, but in others Paradigms such as Imperative Programming there can be a global variable that is used in the function "foo()" that goes op each time it is call. Thus "foo(x) + foo(x)" is not the same as "2*foo(x)" in some cases.
% til her + lidt mere nede
...
\textbf{Characteristic} 		& \rotatebox{90}{Chosen focus for this project} \\ \hline
	...
	\label{tab:langTradeOffs}
\end{table}
\fxfatal{se lige efter}
\begin{itemize}
\item \textbf{Simplicity:} The project languages do not have many ways the same think can be done. This is done to make writing code simple for beginners.

\item \textbf{Orthogonality:} The project languages have a only have 3 types, double, int and char, and 2 special, container and string. it is not possible to make classes or constructs, this is done to help types handling simple for beginners.
